...
void PlaylistComponent::resized()
{
	...
	searchBar.setBounds(colW, 0, colW * 5 - 1, rowH - 5);
	searchBar.setColour(TextEditor::backgroundColourId, Colour(100, 32, 74));
	// set position of table
	tableComponent.setBounds(0, rowH, getWidth(), rowH * 7);
	tableComponent.setColour(ListBox::backgroundColourId, juce::Colours::darkslateblue);
}
...
Component* PlaylistComponent::refreshComponentForCell(int rowNumber, int columnId, bool isRowSelected, Component* existingComponentToUpdate)
{
	...
	if (columnId == 3)
	{
		...
		{
			TextButton* btn = new TextButton{ "Add to Left Deck" };
			String id{ std::to_string(rowNumber) };
			btn->setComponentID(id);
			btn->addListener(this);
			existingComponentToUpdate = btn;
			btn->setColour(TextButton::buttonColourId, juce::Colours::darkblue);
		}
	}
	...
	if (columnId == 4)
	{
		if (existingComponentToUpdate == nullptr)
		{
			TextButton* btn = new TextButton{ "Add to Right Deck" };
